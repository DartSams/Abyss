...
public class player : MonoBehaviour
{
    ...
    public healthBar healthBarScript; //takes in the healthbar script
    public healthBar coolBar;
    RaycastHit hit; //value for raycast that returns what its colliding with
    public Animator anim; //gets the animation component
    public SpriteRenderer sprite; //gets the the sprite renderer component
    ...
    public float maxCoolPoints = 100f; //starting max health
    public float currentCoolPoints; //variable for current health 
    ...
    void Start()
    {
        ...
        healthBarScript.updateHealthbar(maxHealth, currentHealth); //calls the function in the healthbar script to update the healthbar UI Image to be the current health percentage
        currentCoolPoints = 0; 
        coolBar.updateHealthbar(maxCoolPoints, currentCoolPoints);
